...
for fold in $(seq 0 $(($nfolds - 1)))
    ...
    python ./uninmr/infer.py --user-dir  ./uninmr   ${data_path}   --valid-subset valid \
        --results-path ${output_dir}/cv_seed_${cv_seed}_fold_${fold}  --saved-dir $save_dir \
    ...
    done 2>&1 | tee ${output_dir}/infer.log


python ./uninmr/utils/get_result.py --path ${output_dir}  --mode cv --dict "./data/nmrshiftdb2_2018/All/dict.txt" 2>&1 | tee ${output_dir}/result.log
