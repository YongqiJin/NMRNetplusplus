...
for fold in $(seq 0 $(($nfolds - 1)))
    ...
    python ./uninmr/infer.py --user-dir  ./uninmr   ${data_path}   --valid-subset valid \
        ...
        --task uninmr_solv --loss 'atom_regloss_mae' --arch 'unimol_large_solv' \
        --dict-name "dict.txt" --solvent-embed-dim 16\
...
python ./uninmr/utils/get_result.py --path ${save_dir} --file_end "*valid.out.pkl" --mode cv --dict "./data/nmrshiftdb2_2018/All/dict.txt" 2>&1 | tee ${save_dir}/result.log
python ./uninmr/utils/get_result_unlabel.py --path ${save_dir} --file_end "*valid.out.pkl" --mode cv --dict "./data/nmrshiftdb2_2018/All/dict.txt" 2>&1 | tee ${save_dir}/result_unlabel_1.log
...
python ./uninmr/utils/get_result_unlabel.py --path ${save_dir} --file_end "*valid_unlabel.out.pkl" --mode cv --dict "./data/nmrshiftdb2_2018/All/dict.txt" 2>&1 | tee ${save_dir}/result_unlabel_2.log
