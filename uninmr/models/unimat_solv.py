...
class UniMatModelwithSolvent(BaseUnicoreModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=int,
            metavar="H",
            help="solvent embedding dimension",
        )
        parser.add_argument(
            "--solvent-embed-before-backbone",
            type=bool,
            metavar="B",
            help="whether to use the solvent embedding before the backbone",
        )
        parser.add_argument(
            "--solvent-embed-after-backbone",
            type=bool,
            metavar="B",
            help="whether to use the solvent embedding after the backbone",
        )
        parser.add_argument(
            "--solvent-max-types",
            type=int,
            metavar="F",
            help="maximum number of solvent types",
        )
        parser.add_argument(
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.args.solvent_embed_before_backbone:
            if solvent is not None:
                solvent_embeds = self.solvent_embed_full(solvent.long())
                solvent_embeds = solvent_embeds.unsqueeze(1).expand(-1, x.size(1), -1)
                x = x + solvent_embeds
        #    solvent_embeds = self.solvent_embed(src_solvent)
        #    x = x + solvent_embeds
        ...
        if self.args.solvent_embed_after_backbone and self.args.solvent_embed_dim > 0:
            if solvent is not None:
                #solvent = solvent.flatten()
                solvent_embeds = self.solvent_embed(solvent.long())
            #else:
            #    solvent_embeds = torch.zeros(model_input.size(0), self.args.solvent_embed_dim, device=model_input.device, dtype=model_input.dtype)
            solvent_embeds = solvent_embeds.unsqueeze(1).expand(-1, model_input.size(1), -1)
            model_input = self.solvent_denser(torch.cat((model_input, solvent_embeds), dim=2))

            # model_input = model_input + solvent_embeds
    ...
    def register_classification_head(
        ...
        # input_dim=self.args.encoder_embed_dim + extra_dim + self.args.solvent_embed_dim
