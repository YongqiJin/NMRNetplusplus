...
class UniNMRTask(UnicoreTask):
    ...
    def add_args(parser):
        ...
        #     default='cauchy',
        #     type=str,
        #     choices=['cauchy', 'normal', 'laplace'],
        #     help="distribution for prediction loss",
        # )
        # parser.add_argument(
        #     "--p-scale",
        #     default=None,
        #     type=float,
        #     help="initial parameters for prediction loss distribution",
        # )
        # parser.add_argument(
        #     "--T",
        #     default=0,
        #     type=float,
        #     help="temperature for softmax loss, 0 means no temperature",
        # )
        # parser.add_argument(
        #     "--sigma",
        #     default=1.0,
        #     type=float,
        #     help="sigma for gaussian loss, 1.0 means no sigma",
        # )
    ...
    def __init__(self, args, dictionary):
        ...
        if self.args.split_mode =='predefine':
            ...
            if self.args.unlabeled_data:
                ...
                # self.valid_dataset = MergedDataset([self.valid_dataset, self.unlabeled_valid_dataset], [1,0], mode="append")
        # self.args.sigma = args.sigma / self.target_scaler.scaler.scale_.item()
        # logging.info("sigma: {}".format(self.args.sigma))
    ...
    def load_dataset(self, split, **kwargs):
        ...
        # self.split = split
        # if self.args.split_mode != 'predefine':
        # split = "train" #[1128, 1129, 1130,  376, 1131, 1132, 1133,  377]
    ...
    def begin_epoch(self, epoch, model):
        if epoch > 1:
            # self.update_distribution_params(self.valid_residues)
        ...
        # logger.info(f"Epoch {epoch} started. Current distribution ({self.args.p_dist}) params: scale={self.args.p_scale}")
        
    # def update_distribution_params(self, residues):
    #     residues = residues.numpy()

    #     if self.args.p_dist == 'cauchy':
    #         _, scale = stats.cauchy.fit(residues, floc=0)
    #     elif self.args.p_dist == 'normal':
    #         _, scale = stats.norm.fit(residues, floc=0)
    #     elif self.args.p_dist == 'laplace':
    #         _, scale = stats.laplace.fit(residues, floc=0)
            
    #     self.args.p_scale = scale
