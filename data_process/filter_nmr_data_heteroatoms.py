...
def create_atoms_target_mask(atoms, atoms_equi_class, nmr_type):
    ...
    unique_classes = set(atoms_equi_class[i] for i, atom in enumerate(atoms) if atom == nmr_type)
    for equi_class in unique_classes:
        indices = [i for i, cls in enumerate(atoms_equi_class) if cls == equi_class and atoms[i] == nmr_type]
        if indices:
            chosen_index = np.random.choice(indices)
            mask[chosen_index] = 1
    # if np.sum(mask) == 1:
    #     logger.info(f"Warning: Only one target atom found in molecule with atoms {atoms} and equi_class {atoms_equi_class}, sum: {np.sum(mask)}.")
...
def check_shift(extracted_shifts, nmr_type):
    if nmr_type == 'F':
        return all(-300 <= shift <= 300 for shift in extracted_shifts)
    elif nmr_type == 'P':
        return all(-250 <= shift <= 300 for shift in extracted_shifts)
    elif nmr_type == 'B':
        return all(-150 <= shift <= 150 for shift in extracted_shifts)
    elif nmr_type == 'Si':
        return all(-70 <= shift <= 40 for shift in extracted_shifts)

def filter_data(input_path, nmr_type, max_chiral_centers, max_atoms, allowed_atoms, filter_warning):
    ...
    df = df[(df['atom_number_diff_env'] == 1) & (df["nmr_processed"].apply(lambda x: len(x) == 1))]
    df["extracted_shifts"] = df["nmr_processed"].apply(parse_shift)
    df["check_shift"] = df["extracted_shifts"].apply(check_shift, nmr_type=nmr_type)
    df = df[df["check_shift"] == True]
    ...
    df = df[(df['count_mask'] == 1) & (df['count_shifts'] == 1)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nmr_type", '-t', default='F', choices=["F", "B", "P", "Si"], help="NMR type to filter (1H NMR or 13C NMR).")
    parser.add_argument("--max_chiral_centers", '-c', type=int, default=1, help="Maximum number of chiral centers allowed.")
    parser.add_argument("--max_atoms", '-m', type=int, default=512, help="Maximum number of atoms allowed.")
    parser.add_argument("--allowed_atoms", '-a', default=None, help="Allowed atoms for filtering.")
    ...
    filter_data(args.input, args.nmr_type, args.max_chiral_centers, args.max_atoms, allowed_atoms, args.filter_warning)

# python filter_nmr_data_heteroatoms.py -t Si && python filter_nmr_data_heteroatoms.py -t F && python filter_nmr_data_heteroatoms.py -t P && python filter_nmr_data_heteroatoms.py -t B